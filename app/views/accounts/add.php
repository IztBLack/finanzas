<?php require APPROOT . '/views/inc/header.php'; ?>
  ...
  <div class="card shadow-sm border-0">
    <div class="card-header bg-white py-3">
        <h4 class="m-0 font-weight-bold text-primary"><i class="fas fa-plus-circle mr-2"></i> Añadir Cuenta</h4>
        <small class="text-muted">Crea una cuenta o cartera (Ej: Efectivo, Tarjeta BBVA)</small>
    </div>
    ...
    <form action="<?php echo URLROOT; ?>/accounts/add" method="post">
      <div class="row">
        ...
        <div class="col-md-6 form-group mb-4">
          ...
          <input type="text" name="name" class="form-control <?php echo (!empty($data['name_err'])) ? 'is-invalid' : ''; ?>" value="<?php echo $data['name']; ?>" placeholder="Ej: Tarjeta de Débito Citibanamex">
          <span class="invalid-feedback"><?php echo $data['name_err']; ?></span>
        </div>
      </div>

      <div class="form-group mb-4" id="balanceField">
        <label for="initial_balance" class="form-section-label">Balance Inicial o Deuda Actual ($) <sup>*</sup></label>
        <input type="number" step="0.01" name="initial_balance" class="form-control <?php echo (!empty($data['balance_err'])) ? 'is-invalid' : ''; ?>" value="<?php echo $data['initial_balance'] !== '' ? $data['initial_balance'] : '0.00'; ?>">
      ...
      <div id="creditFields" class="p-3 mb-3 rounded" style="background-color:#f8f9fc; display: <?php echo ($data['type'] == 'credit') ? 'block' : 'none'; ?>;">
        ...
        <div class="row">
            <div class="col-md-6 form-group mb-md-0">
                ...
                <input type="number" min="1" max="31" name="cutoff_date" class="form-control <?php echo (!empty($data['type_err']) && empty($data['cutoff_date'])) ? 'is-invalid' : ''; ?>" value="<?php echo $data['cutoff_date']; ?>" placeholder="Ej: 15">
            </div>
            <div class="col-md-6 form-group mb-0">
                <label for="payment_date" class="form-section-label">Día límite de pago <sup>*</sup></label>
                <input type="number" min="1" max="31" name="payment_date" class="form-control <?php echo (!empty($data['type_err']) && empty($data['payment_date'])) ? 'is-invalid' : ''; ?>" value="<?php echo $data['payment_date']; ?>" placeholder="Ej: 5">
            </div>
        </div>
      </div>

      <hr>
      <button type="submit" class="btn btn-success px-4 py-2 font-weight-bold"><i class="fas fa-save mr-2"></i> Guardar Cuenta</button>
    </form>
    </div>
  </div>
  <script>
