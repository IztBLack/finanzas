<?php require APPROOT . '/views/inc/header.php'; ?>
  ...
  <div class="card shadow-sm border-0">
    <div class="card-header bg-white py-3">
        <h4 class="m-0 font-weight-bold text-dark"><i class="fas fa-edit mr-2"></i> Editar Suscripción</h4>
    </div>
    <div class="card-body p-4">
        <form action="<?php echo URLROOT; ?>/subscriptions/edit/<?php echo $data['id']; ?>" method="post">
        ...
        <div class="row">
            ...
            <div class="col-md-6 form-group mb-4">
                <label class="font-weight-bold text-muted small text-uppercase">Estado de la Suscripción: <sup>*</sup></label>
                <select name="status" class="form-control">
                    <option value="active" <?php echo $data['status'] == 'active' ? 'selected' : ''; ?>>Activa</option>
                    <option value="paused" <?php echo $data['status'] == 'paused' ? 'selected' : ''; ?>>Pausada</option>
                </select>
            </div>
        </div>

        <hr>
        <button type="submit" class="btn btn-dark px-4 py-2 font-weight-bold"><i class="fas fa-save mr-2"></i> Actualizar Suscripción</button>
        </form>
    </div>
  </div>
